...
from ciphers.container import Container
...
if __name__ == '__main__':
    ...
    if len(args) == 1 or args[1] not in ['-n', '-f']:
        print("Usage:\n"
              "    python3 -m ciphers.execute -n file1 file2\n"
              "\n"
              "    python3 -m ciphers.execute -f input1 output1 output2\n")
        exit(5)
    elif args[1] == '-n':
        if len(args) == 5:
            cnt = args[2]
            ...
            print("In random generator mode, use\n"
                  "    python3 -m ciphers.cli -n <integer::count of elements> "
                  "<str::output file> <str::output sorted file>\n"
                  "Example:\n"
                  "    python3 -m ciphers.cli -n 1000 outrnd_1.txt outrnd_2.txt")
            exit(3)
    elif args[1] == '-f':
        if len(args) == 5:
            in_file = args[2]
    ...
    try:
        ...
        with open(out_file, 'w') as out:
            ...
            for i, cipher in enumerate(container):
                out.write(f"{i}: {cipher}\n")
